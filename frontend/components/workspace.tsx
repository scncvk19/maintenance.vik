...
function RecordGallery({ section, records, documents, assetName, onEdit, onDelete }: { section: 'contracts' | 'transactions' | 'documents' | 'reminders'; records: Row[]; documents: Row[]; assetName: (id: string | number | null) => string; onEdit: (row: Row) => void; onDelete: (row: Row) => void }) {
    ...
    <div className="main-shell">
      ...
      <main>
        ...
        {!['dashboard','settings','tax'].includes(section) && <>
          ...
          {(['contracts','transactions','documents','reminders'].includes(section)) && (records.length ? <RecordGallery section={section as 'contracts' | 'transactions' | 'documents' | 'reminders'} records={records} documents={documents} assetName={assetName} onEdit={row => setEditor({resource: String(row.resource || section), row})} onDelete={openDelete}/> : <div className="empty panel"><FileText size={30}/><h3>{query || filterAsset || filterKind || filterMonth ? 'Keine passenden Einträge' : 'Hier beginnt deine Übersicht'}</h3><p>{query || filterAsset || filterKind || filterMonth ? 'Passe Suche oder Filter an.' : 'Neue Einträge erscheinen hier, sobald du sie anlegst.'}</p></div>)}
