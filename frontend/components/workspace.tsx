...
async function loadWorkspace() {
  ...
  const [results, dashboard, reminders] = await Promise.all([Promise.all(names.map(n => api<Row[]>(`records/${n}`))), api<Dashboard>('dashboard'), api<Row[]>('reminders')]);
  return { rows: Object.fromEntries([...names.map((n, i) => [n, results[i]]), ['reminders', reminders]]), dashboard };
}
...
export default function Workspace() {
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      <header className="topbar"><div className="breadcrumb"><button className="icon-button mobile-toggle" aria-label="Menü öffnen" onClick={() => setMobile(true)}><Menu/></button><span>Arbeitsplatz</span><ChevronRight size={14}/><strong>{nav.find(n => n[0] === section)?.[1]}</strong></div><div className="top-actions"><select aria-label="Design" value={theme} onChange={e => setTheme(e.target.value)}><option value="light">Hell</option><option value="dark">Dunkel</option></select><button className="icon-button" aria-label="Aufgaben und Termine öffnen" onClick={() => go('tasks')}><Bell size={19}/></button><span className="avatar">V</span></div></header>
      <main>
