...
const descriptions: Record<Section, string> = {
  dashboard: 'Dein Bestand. Deine Termine. Alles an einem Ort.', assets: 'Bestand zentral verwalten.', properties: 'Immobilie auswählen und alle Informationen zentral verwalten.', vehicles: 'Fahrzeuge, Kosten, Termine und Dokumente verwalten.', tax: 'Steuerfälle planen, Unterlagen sammeln und Fristen im Blick behalten.',
...
function RecordGallery({ section, records, documents, assetName, onEdit, onDelete, onAnalyze }: { section: 'contracts' | 'transactions' | 'documents' | 'reminders'; records: Row[]; documents: Row[]; assetName: (id: string | number | null) => string; onEdit: (row: Row) => void; onDelete: (row: Row) => void; onAnalyze?: (row: Row) => void }) {
  ...
  return <div className={`record-gallery ${section}`} aria-label={`${config.label}e`}>
    {records.map(row => {
      ...
      const category = section === 'documents' ? categories[String(row.category)] : section === 'transactions' ? categories[String(row.category)] : section === 'contracts' ? 'Wiederkehrende Zahlung' : reminderIsContract ? 'Vertrag endet' : row.reminder_type === 'document' ? 'Dokumentfrist' : 'Fälliger Eintrag';
...
export default function Workspace() {
  ...
  const [editor, setEditor] = useState<{resource: string; row?: Row; defaultKind?: string; defaultAssetId?: string; defaultDueDate?: string} | null>(null);
  ...
  const [whatsappPreview, setWhatsappPreview] = useState<{counts: Record<string, number>; recipients: number; configured: boolean; template: string | null} | null>(null);
  ...
  const resource = section === 'maintenance' || section === 'tasks' || section === 'tax' ? 'work-items' : section === 'properties' || section === 'vehicles' ? 'assets' : section;
  ...
  function go(next: Section) { setSection(next); location.hash = next; setQuery(''); setFilterAsset(''); setFilterKind(''); setFilterMonth(''); setHistory(false); setMobile(false); setNotice(''); }
  ...
  async function loadWhatsappPreview() {
    setBusy(true); setError('');
    try { setWhatsappPreview(await api('notifications/whatsapp/status')); }
    catch (e) { setError((e as Error).message); } finally { setBusy(false); }
  }
  async function sendWhatsapp() {
    if (!window.confirm('Fällige Erinnerungsanzahlen jetzt über das konfigurierte WhatsApp-Template senden?')) return;
    setBusy(true); setError('');
    try {
      const result = await api<{sent: number; already_sent: number}>('notifications/whatsapp/send', { method: 'POST', headers: { 'X-Confirm-Send': 'SEND_WHATSAPP' } });
      setNotice(`WhatsApp: ${result.sent} gesendet, ${result.already_sent} heute bereits versendet.`);
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main>
        ...
        {section === 'settings' && <div className="settings-grid"><section className="panel settings-panel"><ShieldCheck size={28}/><h2>Deine Daten. Deine Sicherung.</h2><p>Exportiere alle Datensätze und zugehörigen Dokumente/Bilder gemeinsam als ZIP-Datei.</p><button className="primary" disabled={busy} onClick={exportBackup}><Download size={18}/>Backup exportieren</button><hr/><h3>Backup wiederherstellen</h3><p>Das Backup ersetzt den gesamten aktuellen Datenbestand. Exportiere vorher eine Sicherung. Die Datei wird vor dem Import auf Version, Verknüpfungen und Prüfsummen geprüft.</p><label className="upload-field"><Upload size={20}/>Backup auswählen (ZIP, bis 250 MB)<input type="file" accept=".zip" disabled={busy} onChange={e => inspectBackup(e.target.files?.[0])}/></label>{preview && <div className="restore-preview"><h3>Backup geprüft</h3>{Object.entries(preview.counts).map(([name,count]) => <p key={name}>{({assets:'Assets',components:'Räume / Komponenten','work-items':'Wartungen / Aufgaben',transactions:'Finanzeinträge',documents:'Dokumente'} as Record<string,string>)[name]}: <strong>{count}</strong></p>)}<label>Zum Ersetzen WIEDERHERSTELLEN eingeben<input value={confirmation} onChange={e => setConfirmation(e.target.value)}/></label><button className="danger-button" disabled={busy || confirmation !== 'WIEDERHERSTELLEN'} onClick={() => act(async () => {const body = new FormData(); body.set('file', backup!); body.set('confirmation', confirmation); await api('backup/import', {method:'POST',body}); setPreview(null); setBackup(null); setConfirmation('');}, 'Backup erfolgreich wiederhergestellt.')}>Gesamten Bestand ersetzen</button></div>}</section><div><section className="panel settings-panel"><h2>Anbindungen</h2><div className="integration"><strong>Lokale Dokumentenanalyse / OCR</strong><span className="badge open">Verfügbar</span><p>PDFs, Bilder, TXT, CSV und DOCX können lokal analysiert werden. Kategorien werden nur nach deiner Bestätigung gespeichert.</p></div><div className="integration"><strong>Telegram</strong><span className="badge open">Verfügbar</span><p>Erinnerungen können datensparsam versendet werden. Telegram erhält nur Anzahlen, keine Titel, Adressen, Beträge oder Steuerinhalte.</p><div className="record-links"><button className="secondary" disabled={busy} onClick={loadTelegramPreview}>Vorschau laden</button>{telegramPreview?.configured && <button className="primary" disabled={busy || !telegramPreview.telegram_recipients} onClick={sendTelegram}>Jetzt senden</button>}</div>{telegramPreview && <small>{telegramPreview.telegram_recipients} Telegram-Empfänger · {telegramPreview.configured ? "Bot aktiviert" : "Bot noch nicht aktiviert"} · Aufgaben/Wartungen {telegramPreview.counts.work_items || 0}, Verträge {telegramPreview.counts.contracts || 0}, Dokumente {telegramPreview.counts.documents || 0}</small>}</div><div className="integration"><strong>WhatsApp</strong><span className="badge open">Optional verfügbar</span><p>Versand über ein freigegebenes Cloud-API-Template. Übertragen werden nur Erinnerungsanzahlen.</p><div className="record-links"><button className="secondary" disabled={busy} onClick={loadWhatsappPreview}>Status laden</button>{whatsappPreview?.configured && <button className="primary" disabled={busy || !whatsappPreview.recipients} onClick={sendWhatsapp}>Jetzt senden</button>}</div>{whatsappPreview && <small>{whatsappPreview.recipients} WhatsApp-Empfänger · {whatsappPreview.configured ? `Provider aktiviert · Template ${whatsappPreview.template}` : "Provider noch nicht aktiviert"} · Aufgaben/Wartungen {whatsappPreview.counts.work_items || 0}, Verträge {whatsappPreview.counts.contracts || 0}, Dokumente {whatsappPreview.counts.documents || 0}</small>}</div></section><section className="panel settings-panel"><h2>Benachrichtigungsempfänger</h2><p>Hinterlege mehrere Telegram-Chats oder WhatsApp-Rufnummern für spätere Erinnerungen.</p><div className="recipient-list">{(rows["notification-recipients"] || []).map(recipient => <div className="recipient-row" key={recipient.id}><div><strong>{recipient.label}</strong><small>{recipient.channel === "telegram" ? "Telegram" : "WhatsApp"} · {recipient.address}</small></div><span className={`badge ${recipient.active ? "open" : ""}`}>{recipient.active ? "Aktiv" : "Pausiert"}</span><button className="icon-button" aria-label={`${recipient.label} bearbeiten`} onClick={() => setEditor({resource:"notification-recipients", row:recipient})}><Pencil size={16}/></button><button className="icon-button danger" aria-label={`${recipient.label} löschen`} onClick={() => act(() => api(`records/notification-recipients/${recipient.id}`, {method:"DELETE"}), "Empfänger gelöscht.") }><Trash2 size={16}/></button></div>)}</div><button className="secondary" onClick={() => setEditor({resource:"notification-recipients"})}>Empfänger hinzufügen</button></section><section className="panel settings-panel"><h2>Papierkorb</h2><p>Gelöschte Einträge bleiben hier wiederherstellbar, bis du sie endgültig entfernst.</p><div className="recipient-list">{(rows.trash || []).length ? (rows.trash || []).map(item => <div className="recipient-row" key={item.id}><div><strong>{item.label || 'Gelöschter Eintrag'}</strong><small>{item.resource} · {new Date(String(item.deleted_at)).toLocaleString('de-DE')}</small></div><button className="secondary" disabled={busy} onClick={() => act(() => api(`trash/${item.id}/restore`, {method:'POST'}), 'Eintrag wiederhergestellt.')}>Wiederherstellen</button><button className="icon-button danger" disabled={busy} aria-label="Endgültig löschen" onClick={() => { if (window.confirm('Diesen Papierkorb-Eintrag endgültig löschen?')) act(() => api(`trash/${item.id}`, {method:'DELETE'}), 'Papierkorb-Eintrag endgültig gelöscht.'); }}><Trash2 size={16}/></button></div>) : <small>Der Papierkorb ist leer.</small>}</div></section><section className="panel settings-panel"><h2>Aktivitätsverlauf</h2><p>Die letzten Änderungen an deinem Bestand.</p><div className="activity-list">{(rows.activity || []).slice(0, 30).map(item => <div className="activity-row" key={item.id}><span className="badge open">{({created:'Angelegt',updated:'Geändert',deleted:'Gelöscht',restored:'Wiederhergestellt',purged:'Endgültig gelöscht'} as Record<string,string>)[String(item.action)] || item.action}</span><div><strong>{item.label || item.resource}</strong><small>{new Date(String(item.created_at)).toLocaleString('de-DE')} · {item.resource}</small></div></div>)}{!(rows.activity || []).length && <small>Noch keine protokollierten Änderungen.</small>}</div></section><section className="panel settings-panel"><h2>Über diesen Arbeitsbereich</h2><p>Version 0.1 · Lokaler Betrieb; optional mit individuellen Admin- und Leser-Konten.</p><p>Finanzen dienen der Übersicht, nicht der vollständigen Buchhaltung. Daten bleiben in deiner Installation.</p></section></div></div>}
        </>}
