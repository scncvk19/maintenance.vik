  ...
  return <div className="overlay asset-detail-overlay" onClick={event => { if (event.target === event.currentTarget) close(); }}>
    <section className="asset-detail" role="dialog" aria-modal="true" aria-labelledby="asset-detail-title">
      ...
      <div className="asset-detail-grid">
        ...
        <section className="detail-card span-two"><h3><CircleDollarSign size={17}/>Finanzeinträge <small>{transactions.filter(item => item.asset_id === asset.id).length}</small></h3>{transactions.filter(item => item.asset_id === asset.id).length ? <div className="compact-list">{transactions.filter(item => item.asset_id === asset.id).sort((a, b) => String(b.booked_date).localeCompare(String(a.booked_date))).map(item => <span key={item.id}><b>{item.title}</b><small>{day(item.booked_date)} · {transactionCategories[String(item.category)] || 'Sonstiges'} · {item.component_id ? `${components.find(component => component.id === item.component_id)?.name || 'Teilbereich'} · ` : ''}{item.direction === 'income' ? '+' : '−'}{money(Number(item.amount_cents))}</small></span>)}</div> : <p>Keine Einnahmen oder Ausgaben hinterlegt.</p>}</section>
      </div>
    </section>
  </div>;
}
